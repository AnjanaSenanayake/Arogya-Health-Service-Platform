...
User.getAll = result => {
  sql.query("SELECT User.*,UserResidentialData.*,DivisionalSecretariats.*,DivisionalSecretariats.DivisionalSecretariatName,GramaNiladhariDivisions.GNDivisionName FROM User,UserContactData,UserResidentialData,DivisionalSecretariats,GramaNiladhariDivisions WHERE DivisionalSecretariats.DSID = UserResidentialData.DSDivision AND GramaNiladhariDivisions.GNID = UserResidentialData.GNDivision", (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
